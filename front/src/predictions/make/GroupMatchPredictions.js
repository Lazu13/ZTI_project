...
import {getGroupMatches, makePrediction} from '../../util/APIUtils'
...
class GroupMatchPredictions extends Component {
    constructor(props) {
        super(props);
        this.state = {
            matches: [],
    ...
    loadMatches(page = 0, size = MATCHES_LIST_SIZE) {
        let promise = getGroupMatches(page, size);
    ...
    makePrediction(predictionData) {
        if (this.isDisabled(predictionData)) {
            notification.error({
                message: 'ZTI',
                description: "Musisz uzupełnic wymagane pola"
            })
        } else {
            const preparedData = {
    ...
    render() {
        ...
        this.state.matches.forEach((match, matchIndex) => {
            ...
            dataSource.push({
                ...
                home_team: match.teamH.name,
                away_team: match.teamA.name,
                home_team_id: match.teamH.id,
                away_team_id: match.teamA.id,
                result_home: resultH,
                result_away: resultA,
        ...
        const grouped = this.groupBy(filtered, match => match.group);
...
export default withRouter(GroupMatchPredictions)